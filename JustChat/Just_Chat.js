...
import { fetchEventFromFirebase, fetchEvents, saveEvent, updateEvent, deleteEvent, fetchEventsForToday, saveResponse, getResponsesByDateAndTitle, db, getCurrentUserId } from "../backend/firebase.js";
import { calendar } from "../Calendar/calendarMain.js"; // Import calendar instance
const endpoint = "https://my-backend-three-pi.vercel.app/api/chat"; // Updated chat API endpoint
import { parse, isValid, format } from "https://cdn.skypack.dev/date-fns";
...
export async function initializeChat(aiResponseContext) {
  try {
    ...
    if (!chatState.userId) {
      ...
      await initializeAuthState(); // Ensure auth state is initialized
    ...
    // Determine the system message based on addEventMode
    let systemMessage = null;
    if (!addEventMode) {
      systemMessage = {
      role: "system",
      content:
        "You are a helpful and friendly assistant that aims to offer help to the user in the best way it can. If you need more information, feel free to ask!",
      };
    }

    // Set up the initial system message only if chatState.messages is empty
    ...
    console.log(
      "Chat initialized successfully with context:",
      JSON.stringify(chatState.messages, null, 2)
    );
  } catch (error) {
