    ...
    async function populateModal(event) {
      ...
      if (!saveButton) {
        ...
        saveButton.addEventListener("click", async () => {
          ...
          const aiResponseEnabled = aiResponseCheckbox ? aiResponseCheckbox.checked : false;
...
document.querySelector("#saveEvent").addEventListener("click", async () => {
  ...
  const aiResponseEnabled = document.querySelector("#receiveAIResponse").checked;
...
saveButton.addEventListener("click", (event) => {
  ...
  console.log("Receive AI Response Checked:", receiveAIResponseCheckbox.checked);

  // Show the first popup immediately
  showDragSuccess();

  // If the event is today AND receiveAIResponse is checked, show AI processing popup after a delay
  if (eventDate === today && receiveAIResponseCheckbox.checked) {
    console.log("Conditions met - showing AI processing popup");
    setTimeout(() => {
      showAIProcessingPopup();
    }, 2000); // 1-second delay before showing AI processing popup
  } else {
